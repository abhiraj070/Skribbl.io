export default function WordDisplay({ length, revealedWord, variant = "default" }) {
  ...
  const revealedCls = variant === "drawer" ? "text-brand-300" : "text-success";
  return (
    <div className="flex flex-col items-center gap-1">
        ...
        {chars.map((c, i) => (
          <span
            ...
            className={`inline-block w-5 sm:w-6 text-center text-xl sm:text-2xl font-bold ${
              revealedWord ? revealedCls : "text-white border-b-2 border-white/40"
            }`}
          >
            {c === " " ? "\u00A0" : c}
          </span>
      ...
      <div className="text-xs uppercase tracking-widest text-slate-400">{length} letters</div>
    </div>
  );
}
